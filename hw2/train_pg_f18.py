...
def build_mlp(input_placeholder, output_size, scope, n_layers, size, activation=tf.tanh, output_activation=None):
    ...
    output = tf.layers.dense(x, output_size, activation=output_activation)
    return output
...
class Agent(object):
    ...
    def policy_forward_pass(self, sy_ob_no):
        ...
        """
        # raise NotImplementedError
        out = build_mlp(sy_ob_no, self.ac_dim, 'net', self.n_layers, self.size)
        if self.discrete:
            # YOUR_CODE_HERE
            sy_logits_na = out
            return sy_logits_na
            ...
            # YOUR_CODE_HERE
            sy_mean = out
            std = tf.get_variable('std', shape=[self.ac_dim], dtype=tf.float32)
            sy_logstd = tf.exp(std)
    ...
    def get_log_prob(self, policy_parameters, sy_ac_na):
        ...
        if self.discrete:
            ...
            sy_logprob_n = tf.contrib.distributions.MultivariateNormalDiag(loc=sy_mean,
                    scale_diag=sy_logstd).log_prob(sy_ac_na)
        return sy_logprob_n
    ...
    def build_computation_graph(self):
        ...
        if self.nn_baseline:
            # raise NotImplementedError
            # why not share network with before?
    ...
    def sum_of_rewards(self, re_n):
        ...
        # YOUR_CODE_HERE
        q_n = []
        for res in re_n:  # for every trajectory
            steps = np.arange(len(res))
            gammas = np.power(self.gamma, steps)
            if self.reward_to_go:
                qs = [np.sum(gammas[:-t or None] * res[t:]) for t in steps]
            else:
                qs = [np.sum(gammas * res)] * len(res)
            q_n += qs
    ...
    def compute_advantage(self, ob_no, q_n):
        ...
        if self.nn_baseline:
            ...
            b_n = b_n * q_n.std() + q_n.mean()  # rescale
            adv_n = q_n - b_n
        else:
            adv_n = q_n.copy()
        return adv_n
    ...
    def estimate_return(self, ob_no, re_n):
        ...
        if self.normalize_advantages:
            ...
            # raise NotImplementedError # YOUR_CODE_HERE
...
def train_PG(
    ...
    for itr in range(n_iter):
        print("********** Seed(%s) Iteration %i ************"%(seed, itr))
...
def main():
    ...
    parser.add_argument('--loop', '-lp', action='store_true')
